/**
 * Module     : ckETH.mo (ICTokens version)
...
//record { totalSupply=0; decimals=18; fee=100000000000; name=opt "ckETH (PR) on IC"; symbol=opt "ckETHT"; metadata=null; founder=null;}  
shared(installMsg) actor class DRC20(initArgs: Types.InitArgs) = this {
    ...
    * State Variables 
    */
    private var standard_: Text = "drc20; icrc1; icrc2; ictokens"; 
    ...
    */
    ...
    public query func icrc1_minting_account() : async ?Account{
        if (minters.size() > 0){
            return ?{owner = minters[0]; subaccount = null;};
        }else{
            return null;
        };
    };
    ...
    public shared(msg) func icrc1_transfer(_args: TransferArgs) : async ({ #Ok: Nat; #Err: TransferError; }) {
        ...
        if (_onlyMiner(msg.caller) and to == _getAccountIdFromPrincipal(msg.caller, null)){ // burn
            ...
            if(not(_checkFee(from, 0, value))){
                ...
                // switch(res){
                //     case(#ok(v)){ ignore _chargeFee(from, 100);};
                //     case(#err(v)){};
                // };
            };
    ...
    * Extended functions
    */
    // private var MINING_CANISTER = "pm5h6-jqaaa-aaaak-aejja-cai";
    private stable var minters: [Principal] = [Principal.fromText("pm5h6-jqaaa-aaaak-aejja-cai")]; 
    private func _onlyMiner(_caller: Principal) : Bool { 
        return Option.isSome(Array.find(minters, func (t: Principal): Bool{ t == _caller }));
    };
    public shared(msg) func ictokens_addMinter(_minter: Principal) : async Bool{
        assert(_onlyOwner(msg.caller));
        minters := AID.arrayAppend(minters, [_minter]);
        return true;
    };
    public shared(msg) func ictokens_removeMinter(_minter: Principal) : async Bool{ 
        assert(_onlyOwner(msg.caller));
        minters := Array.filter(minters, func (t: Principal): Bool{ t != _minter });
        return true;
    };
    public query func ictokens_minters() : async [Principal]{ 
        return minters;
    };
    ...
    /// burn
    public shared(msg) func ictokens_burn(_value: Amount, _nonce: ?Nonce, _sa: ?Sa, _data: ?Data) : async (result: TxnResult) { //icl
